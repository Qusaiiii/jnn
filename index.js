...
if(message.content.split(' ')[0] == '-bc') {
    if (!args[1]) {
message.channel.send("**| Qazem. | Usage: -bc <message>**");
return;
}
...
const cuttweet = [
 ...
 client.on('message', message => {
  ...
  .setColor('RANDOM')
   .setThumbnail(message.author.avatarURL) 
 .addField('Qazem. v0.1 | لعبه كت تويت' ,
...
client.on('message', message => { 
  var whitelisted = "474172469566111745"// ايديك
...
.setTitle("Qazem. v0.1 | لعبة ماين كرافت ..")
.setColor('RANDOM')
...
__الإقتراح__ :  \`\`\`${args.join(" ")}\`\`\`**`)
           client.channels.get("475999476188708865").send(embed)
...
client.login('NDc5NjU2MTQ1NTE5MjQ3MzY1.DlcrtA.h_Y0rfDUUz-Y6sv9DmXBi3cf7Rg');
